...
class OpenChannelChatAdapter(
    ...
    inner class GroupChatSendViewHolder(private val binding: ListItemChatSendBinding) :
        ...
        fun bind(message: BaseMessage) {
            if (message.sendingStatus == SendingStatus.SUCCEEDED) {
                ...
                binding.chatBubbleSend.findViewById<TextView>(R.id.textview_msg).apply {
                    setTextColor(data.getInt("color"))
                    textSize = data.getDouble("textSize").toFloat()
                    letterSpacing = data.getDouble("letterSpacing").toFloat()
                    backgroundTintList = ColorStateList.valueOf(data.getInt("background"))
                }
            } else {
    ...
    inner class GroupChatReceiveViewHolder(private val binding: ListItemChatReceiveBinding) :
        ...
        fun bind(message: BaseMessage) {
            ...
            val data = JSONObject(message.data)
            binding.chatBubbleReceive.findViewById<TextView>(R.id.textview_msg).apply {
                setTextColor(data.getInt("color"))
                textSize = data.getDouble("textSize").toFloat()
                letterSpacing = data.getDouble("letterSpacing").toFloat()
                backgroundTintList = ColorStateList.valueOf(data.getInt("background"))
