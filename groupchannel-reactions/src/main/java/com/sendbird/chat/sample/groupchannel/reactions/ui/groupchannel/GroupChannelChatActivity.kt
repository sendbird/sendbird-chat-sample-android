...
import com.sendbird.chat.sample.groupchannel.reactions.R
import com.sendbird.chat.sample.groupchannel.reactions.databinding.ActivityGroupChannelChatBinding
import com.sendbird.chat.sample.groupchannel.reactions.ui.user.ChatMemberListActivity
import com.sendbird.chat.sample.groupchannel.reactions.ui.user.SelectUserActivity
...
class GroupChannelChatActivity : AppCompatActivity() {
    ...
    private fun initRecyclerView() {
        adapter = GroupChannelChatAdapter({ baseMessage, view ->
            view.setOnCreateContextMenuListener { contextMenu, _, _ ->
                ...
                val reactMenu = contextMenu.add(Menu.NONE, 3, 3, "React")
                reactMenu.setOnMenuItemClickListener {
                    openReactDialog(baseMessage)
                    true
                }
            }
        }, {
    ...
    private fun openReactDialog(message: BaseMessage) {
        ReactDialog().apply {
            onReactClicked = { addReaction(message, it) }
            show(supportFragmentManager, "React_Dialog")
        }
    }

    private fun addReaction(message: BaseMessage, reaction: String) {
        currentGroupChannel?.addReaction(message, reaction) handler@{ event, e ->
            if (e != null) {
                e.printStackTrace()
    ...
    private fun setReactionHandler() {
        SendbirdChat.addChannelHandler(ReactionHandlerKey, object : GroupChannelHandler() {
            ...
            override fun onReactionUpdated(channel: BaseChannel, reactionEvent: ReactionEvent) {
                ...
                val message = adapter.currentList.firstOrNull { it.messageId == messageId } ?: return
    ...
    private fun createMessageCollection(timeStamp: Long) {
        ...
        val messageListParams = MessageListParams().apply {
            ...
            nextResultSize = 20
            messagePayloadFilter = MessagePayloadFilter.Builder().setIncludeReactions(true).build()
        }
    ...
    override fun onDestroy() {
        ...
        messageCollection?.dispose()
        SendbirdChat.autoBackgroundDetection = true
        SendbirdChat.removeChannelHandler(ReactionHandlerKey)
    }
