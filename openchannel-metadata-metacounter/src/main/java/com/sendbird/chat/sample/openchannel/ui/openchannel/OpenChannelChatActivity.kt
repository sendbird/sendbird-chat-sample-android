...
class OpenChannelChatActivity : AppCompatActivity() {
    ...
    private fun checkForDarkMode() {
        currentOpenChannel?.getMetaData(listOf("dark_mode")) { metaData, e ->
            if (e != null) {
                e.printStackTrace()
                return@getMetaData
            }
            val darkMode = metaData?.get("dark_mode")?.let { it.toBoolean() } ?: false
    ...
    private fun changeDarkMode() {
        currentOpenChannel?.getMetaData(listOf("dark_mode")) { metaData, e ->
            ...
            if (metaData?.isEmpty() == true) {
                currentOpenChannel?.createMetaData(mapOf("dark_mode" to true.toString())) { meta, exception ->
                    if (exception != null) {
                        exception.printStackTrace()
                        return@createMetaData
                    }
                    changeBackground(meta?.get("dark_mode")?.let { it.toBoolean() } ?: false)
                }
            } else {
                val darkMode = (!(metaData?.get("dark_mode")?.let { it.toBoolean() } ?: false)).toString()
                currentOpenChannel?.updateMetaData(mapOf("dark_mode" to darkMode)) { meta, exception ->
                    if (exception != null) {
                        exception.printStackTrace()
    ...
    private fun changeBackground(darkMode: Boolean) {
        binding.recyclerviewChat.setBackgroundColor(if (darkMode) Color.LTGRAY else Color.WHITE)
    }
    ...
    private fun updateMetaCounters(messages: List<BaseMessage>) {
        val files = messages
            .filterIsInstance(FileMessage::class.java)
            .groupBy { it.type }
            .map { it.key to it.value.size }
            .toMap()
        currentOpenChannel?.createMetaCounters(files) { _, e ->
            if (e != null) {
                e.printStackTrace()
                Log.e("META_COUNTERS", "Operation failed")
                currentOpenChannel?.updateMetaCounters(files) { _, updateError ->
                    if (updateError != null) {
    ...
    private fun sendFileMessage(imgUri: Uri?) {
        ...
        if (fileInfo != null) {
            ...
            val pendingMessage = channel.sendFileMessage(
                ...
                checkForMetaCounters(fileMessage?.type ?: "")
            }
            //pending
            adapter.addPendingMessage(pendingMessage)
            recyclerObserver.scrollToBottom(true)
    ...
    private fun checkForMetaCounters(mimeType: String) {
        currentOpenChannel?.getMetaCounters(listOf(mimeType)) { map, e ->
            if (e != null) {
                e.printStackTrace()
                return@getMetaCounters
            }
            if (map?.isEmpty() == true) {
                createMetaCounters(mimeType)
            } else {
                increaseCounterForFiles(mimeType)
            }
        }
    }

    private fun createMetaCounters(mimeType: String) {
        currentOpenChannel?.createMetaCounters(mapOf(mimeType to 1)) { metaCounters, e ->
            if (e != null) {
                e.printStackTrace()
                showToast("Failed to create meta counters")
            }
            showToast("There are ${metaCounters?.get(mimeType)} files of type $mimeType")
        }
    }

    private fun increaseCounterForFiles(mimeType: String) {
        val channel = currentOpenChannel ?: return
        channel.increaseMetaCounters(mapOf(mimeType to 1)) { metaCounterMap, e ->
            if (e != null) {
                e.printStackTrace()
                showToast("Operation Failed")
                return@increaseMetaCounters
            }
            showToast("There are ${metaCounterMap?.get(mimeType)} files of type $mimeType")
        }
    }
