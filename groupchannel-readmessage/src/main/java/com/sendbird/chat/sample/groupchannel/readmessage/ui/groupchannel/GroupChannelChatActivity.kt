package com.sendbird.chat.sample.groupchannel.readmessage.ui.groupchannel
...
class GroupChannelChatActivity : AppCompatActivity() {
    ...
    private fun initRecyclerView() {
        adapter = GroupChannelChatAdapter({ baseMessage, view ->
            view.setOnCreateContextMenuListener { contextMenu, _, _ ->
                ...
                val messageReadUserMenu =
                    contextMenu.add(Menu.NONE, 3, 3, getString(R.string.read_users))
                messageReadUserMenu.setOnMenuItemClickListener {
                    displayMessageReadUsers(baseMessage)
                    return@setOnMenuItemClickListener true
                }
            }
        }, {
    ...
    private fun displayMessageReadUsers(message: BaseMessage) {
        val channel = currentGroupChannel ?: return
        val users = channel
            .getReadMembers(message, true)
            .joinToString(separator = ",") { it.nickname }
        showToast("$users have read this message")
    ...
    companion object {
        private val ReadStatusChannelHandlerId = UUID.randomUUID().toString()
    }
}