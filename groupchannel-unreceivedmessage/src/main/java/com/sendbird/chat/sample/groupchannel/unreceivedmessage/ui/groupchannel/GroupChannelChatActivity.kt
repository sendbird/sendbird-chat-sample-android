package com.sendbird.chat.sample.groupchannel.unreceivedmessage.ui.groupchannel
...
import com.sendbird.chat.sample.groupchannel.unreceivedmessage.ui.DeliverMessageHandler
import com.sendbird.chat.sample.groupchannel.unreceivedmessage.ui.user.ChatMemberListActivity
import com.sendbird.chat.sample.groupchannel.unreceivedmessage.ui.user.SelectUserActivity
import java.util.*
...
class GroupChannelChatActivity : AppCompatActivity() {
    ...
    private fun initRecyclerView() {
        adapter = GroupChannelChatAdapter({ baseMessage, view ->
            view.setOnCreateContextMenuListener { contextMenu, _, _ ->
                ...
                val deliveryStatus =
                    contextMenu.add(Menu.NONE, 3, 3, getString(R.string.delivery_status))
    ...
    private fun getChannel(channelUrl: String?) {
        ...
        GroupChannel.getChannel(
            ...
            if (groupChannel != null) {
                ...
                setupDeliveryStatusHandler()
            }
        }
    }

    /**
     * Setup channel handler to receive live updates about changes in channel message delivery status
     */
    private fun setupDeliveryStatusHandler() {
        SendbirdChat.addChannelHandler(UndeliveredMessageHandlerId, DeliverMessageHandler {
            if (it.url != currentGroupChannel?.url) return@DeliverMessageHandler
            adapter.notifyDataSetChanged()
        })
    }

    private fun checkMessageDeliveryStatus(message: BaseMessage) {
        val deliveryCount = currentGroupChannel?.getUndeliveredMemberCount(message)
        val textToShow =
            if (deliveryCount == 0) "Message was delivered to all members" else "Message was not delivered to $deliveryCount members"
    ...
    companion object {
        private val UndeliveredMessageHandlerId = UUID.randomUUID().toString()
    }
}