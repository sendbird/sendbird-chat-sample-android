...
import com.sendbird.chat.sample.groupchannel.categorizechannels.util.toChatTime
...
class GroupChannelListAdapter(private val listener: OnItemClickListener) :
    ...
    inner class GroupChannelListViewHolder(private val binding: ListItemChannelBinding) :
        ...
        // GroupChannel has a customType property that can be used to categorize channels.
        fun bind(groupChannel: GroupChannel) {
            ...
            var title =
                if (groupChannel.name.isBlank() || groupChannel.name == TextUtils.CHANNEL_DEFAULT_NAME)
                    ...
                    groupChannel.name
            // If the channel has a customType, display it in the title.
            if (groupChannel.customType != ""){
                title += " (${groupChannel.customType})"
            }
